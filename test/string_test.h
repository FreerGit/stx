...
void string_test(void) {
  ...
  ASSERT_TRUE(view.len == count_of("ello") - 1); // One less because the view is _not_ null-terminated
  ...
  ASSERT_TRUE(s8_view_s1.len == count_of("llo") - 1); // One less because the view is _not_ null-terminated
  ASSERT_TRUE(memeql(s8_view_s2.data, "World!", s8_view_s2.len));
  ASSERT_TRUE(s8_view_s2.len == count_of("World!") - 1); // One less because the view is _not_ null-terminated
